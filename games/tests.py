...
class GameCategoryTests(APITestCase):
    ...
    def test_filter_game_category_by_name(self):
        ...
        """
        game_category_name_1 = 'First game category name'
        self.create_game_category(game_category_name_1)
        game_category_name_2 = 'Second game category name'
        self.create_game_category(game_category_name_2)
        filter_by_name = {'name': game_category_name_1}
        url = '{0}?{1}'.format(reverse('gamecategory-list'), urlencode(filter_by_name))
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], 1)
        self.assertEqual(response.data['results'][0]['name'], game_category_name_1)
